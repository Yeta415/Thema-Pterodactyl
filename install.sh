...
check_token() {
  ...
  if [ "$USER_TOKEN" = "AutoinstallbyRinDev" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di Sanka Vollerei${NC}"
    echo -e "${YELLOW}TELEGRAM : @RinDev${NC}"
    echo -e "${YELLOW}WHATSAPP : +6281248136795${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 50K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© RinDev ${NC}"
    exit 1
  fi
  clear
}
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer RinDev   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram :Yedamora      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Credit  : Rindec  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by ElainaStore  ${NC}"
